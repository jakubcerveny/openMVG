...
int main(int argc, char **argv)
{
  ...
  try {
      ...
      << "\t 1: Pinhole\n"
      << "\t 2: Pinhole radial k1\n"
      << "\t 3: Pinhole radial k3 (default)\n"
      << "\t 4: Pinhole radial k4\n"
      << "\t 5: Pinhole brown k3 t2\n"
      << "\t 6: Pinhole brown k4 t4\n"
      << "\t 7: Pinhole rational k6 t2\n"
      << "\t 8: Pinhole with a simple Fish-eye distortion\n"
      << "\t 9: Spherical camera\n"
    ...
    ++iter_image, ++my_progress_bar )
  {
    ...
    if (focal > 0 && ppx > 0 && ppy > 0 && width > 0 && height > 0)
    {
      ...
      switch (e_User_camera_model)
      {
        ...
        break;
        case PINHOLE_CAMERA_RADIAL4:
          intrinsic = std::make_shared<Pinhole_Intrinsic_Radial_K4>
