...
/**
 * @brief Ceres functor to use a Pinhole_Intrinsic_Radial_K4
 *
 *  Data parameter blocks are the following <2,7,6,3>
 *  - 2 => dimension of the residuals,
 *  - 6 => the intrinsic data block [focal, principal point x, principal point y, K1, K2, K3, K4],
 *  - 6 => the camera extrinsic data block (camera orientation and position) [R;t],
 *         - rotation(angle axis), and translation [rX,rY,rZ,tx,ty,tz].
 *  - 3 => a 3D point data block.
 *
 */
struct ResidualErrorFunctor_Pinhole_Intrinsic_Radial_K4
{
  explicit ResidualErrorFunctor_Pinhole_Intrinsic_Radial_K4(const double* const pos_2dpoint)
  :m_pos_2dpoint(pos_2dpoint)
  {
  }
    ...
    T* out_residuals) const
  {
    ...
    const T r6 = r4 * r2;
    const T r8 = r4 * r4;
    const T r_coeff = (1.0 + k1 * r2 + k2 * r4 + k3 * r6 + k4 * r8);
  ...
  )
  {
    if (weight == 0.0)
    {
      return
        (new ceres::AutoDiffCostFunction
          <ResidualErrorFunctor_Pinhole_Intrinsic_Radial_K4, 2, 7, 6, 3>(
            new ResidualErrorFunctor_Pinhole_Intrinsic_Radial_K4(observation.data())));
    }
    else
    {
      return
        (new ceres::AutoDiffCostFunction
          <WeightedCostFunction<ResidualErrorFunctor_Pinhole_Intrinsic_Radial_K4>, 2, 7, 6, 3>
          (new WeightedCostFunction<ResidualErrorFunctor_Pinhole_Intrinsic_Radial_K4>
            (new ResidualErrorFunctor_Pinhole_Intrinsic_Radial_K4(observation.data()), weight)));
    }
  }
...
/**
 * @brief Ceres functor with constrained 3D points to use a Pinhole_Intrinsic_Brown_K4_T4
 *
 *  Data parameter blocks are the following <2,11,6,3>
 *  - 2 => dimension of the residuals,
 *  - 11 => the intrinsic data block [focal, principal point x, principal point y, K1-K4, T1-T4],
 *  - 6 => the camera extrinsic data block (camera orientation and position) [R;t],
 *         - rotation(angle axis), and translation [rX,rY,rZ,tx,ty,tz].
 *  - 3 => a 3D point data block.
 *
 */
struct ResidualErrorFunctor_Pinhole_Intrinsic_Brown_K4_T4
{
  explicit ResidualErrorFunctor_Pinhole_Intrinsic_Brown_K4_T4(const double* const pos_2dpoint)
  :m_pos_2dpoint(pos_2dpoint)
  {
  }
  ...
  /**
   * @param[in] cam_intrinsics: Camera intrinsics( focal, principal point [x,y], k1, k2, k3, k4, t1, t2, t3, t4 )
   * @param[in] cam_extrinsics: Camera parameterized using one block of 6 parameters [R;t]:
   *   - 3 for rotation(angle axis), 3 for translation
   * @param[in] pos_3dpoint
   * @param[out] out_residuals
    ...
    T* out_residuals) const
  {
    ...
    const T& t1 = cam_intrinsics[OFFSET_DISTO_T1];
    const T& t2 = cam_intrinsics[OFFSET_DISTO_T2];
    const T& t3 = cam_intrinsics[OFFSET_DISTO_T3];
    const T& t4 = cam_intrinsics[OFFSET_DISTO_T4];
    ...
    const T r6 = r4 * r2;
    const T r8 = r4 * r4;
    const T r_coeff1 = (1.0 + k1 * r2 + k2 * r4 + k3 * r6 + k4 * r8);
    const T r_coeff2 = (1.0 + t3 * r2 + t4 * r4);
    const T t_x = (t2 * (r2 + 2.0 * x_u * x_u) + 2.0 * t1 * x_u * y_u) * r_coeff2;
    const T t_y = (t1 * (r2 + 2.0 * y_u * y_u) + 2.0 * t2 * x_u * y_u) * r_coeff2;

    Eigen::Map<Eigen::Matrix<T, 2, 1>> residuals(out_residuals);
    residuals << principal_point_x + (projected_point.x() * r_coeff1 + t_x) * focal - m_pos_2dpoint[0],
                 principal_point_y + (projected_point.y() * r_coeff1 + t_y) * focal - m_pos_2dpoint[1];
  ...
  )
  {
    if (weight == 0.0)
    {
      return
        (new ceres::AutoDiffCostFunction
          <ResidualErrorFunctor_Pinhole_Intrinsic_Brown_K4_T4, 2, 11, 6, 3>(
            new ResidualErrorFunctor_Pinhole_Intrinsic_Brown_K4_T4(observation.data())));
    }
    else
    {
      return
        (new ceres::AutoDiffCostFunction
          <WeightedCostFunction<ResidualErrorFunctor_Pinhole_Intrinsic_Brown_K4_T4>, 2, 11, 6, 3>
          (new WeightedCostFunction<ResidualErrorFunctor_Pinhole_Intrinsic_Brown_K4_T4>
            (new ResidualErrorFunctor_Pinhole_Intrinsic_Brown_K4_T4(observation.data()), weight)));
    }
  }
...
/**
 * @brief Ceres functor with constrained 3D points to use a Pinhole_Intrinsic_Rational_T2
 *
 *  Data parameter blocks are the following <2,11,6,3>
 *  - 2 => dimension of the residuals,
 *  - 11 => the intrinsic data block [focal, principal point x, principal point y, K1-K6, T1-T2],
 *  - 6 => the camera extrinsic data block (camera orientation and position) [R;t],
 *         - rotation(angle axis), and translation [rX,rY,rZ,tx,ty,tz].
 *  - 3 => a 3D point data block.
 *
 */
struct ResidualErrorFunctor_Pinhole_Intrinsic_Rational_T2
{
  explicit ResidualErrorFunctor_Pinhole_Intrinsic_Rational_T2(const double* const pos_2dpoint)
  :m_pos_2dpoint(pos_2dpoint)
  {
  }
  ...
  /**
   * @param[in] cam_intrinsics: Camera intrinsics( focal, principal point [x,y], k1, k2, k3, k4, k5, k6, t1, t2 )
   * @param[in] cam_extrinsics: Camera parameterized using one block of 6 parameters [R;t]:
   *   - 3 for rotation(angle axis), 3 for translation
   * @param[in] pos_3dpoint
   * @param[out] out_residuals
    ...
    T* out_residuals) const
  {
    ...
    const T& k3 = cam_intrinsics[OFFSET_DISTO_K3];
    const T& k4 = cam_intrinsics[OFFSET_DISTO_K4];
    const T& k5 = cam_intrinsics[OFFSET_DISTO_K5];
    const T& k6 = cam_intrinsics[OFFSET_DISTO_K6];
    const T& t1 = cam_intrinsics[OFFSET_DISTO_T1];
    const T& t2 = cam_intrinsics[OFFSET_DISTO_T2];
    ...
    const T rad = (1.0 + k1*r2 + k2*r4 + k3*r6) / (1.0 + k4*r2 + k5*r4 + k6*r6);
    const T t_x = t2*(r2 + 2.0*x_u*x_u) + 2.0*t1*x_u*y_u;
    const T t_y = t1*(r2 + 2.0*y_u*y_u) + 2.0*t2*x_u*y_u;

    Eigen::Map<Eigen::Matrix<T, 2, 1>> residuals(out_residuals);
    residuals << principal_point_x + (x_u*rad + t_x)*focal - m_pos_2dpoint[0],
                 principal_point_y + (y_u*rad + t_y)*focal - m_pos_2dpoint[1];
  ...
  )
  {
    if (weight == 0.0)
    {
      return
        (new ceres::AutoDiffCostFunction
          <ResidualErrorFunctor_Pinhole_Intrinsic_Rational_T2, 2, 11, 6, 3>(
            new ResidualErrorFunctor_Pinhole_Intrinsic_Rational_T2(observation.data())));
    }
    else
    {
      return
        (new ceres::AutoDiffCostFunction
          <WeightedCostFunction<ResidualErrorFunctor_Pinhole_Intrinsic_Rational_T2>, 2, 11, 6, 3>
          (new WeightedCostFunction<ResidualErrorFunctor_Pinhole_Intrinsic_Rational_T2>
            (new ResidualErrorFunctor_Pinhole_Intrinsic_Rational_T2(observation.data()), weight)));
    }
  }
