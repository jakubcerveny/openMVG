...
// Copyright (c) 2015 Jakub Cerveny, Pierre Moulon.

// This Source Code Form is subject to the terms of the Mozilla Public
// License, v. 2.0. If a copy of the MPL was not distributed with this
// file, You can obtain one at http://mozilla.org/MPL/2.0/.

#ifndef OPENMVG_CAMERAS_CAMERA_PINHOLE_RATIONAL_HPP
#define OPENMVG_CAMERAS_CAMERA_PINHOLE_RATIONAL_HPP
...
/**
* @brief Implement a Pinhole camera with a 6 radial distortion coefficients and 2 tangential distortion coefficients.
* \f$ x_d = x_u (1 + K_1 r^2 + K_2 r^4 + K_3 r^6) / (1 + K_4 r^2 + K_5 r^4 + K_6 r^6) + (T_2 (r^2 + 2 x_u^2) + 2 T_1 x_u y_u) \f$
* \f$ y_d = y_u (1 + K_1 r^2 + K_2 r^4 + K_3 r^6) / (1 + K_4 r^2 + K_5 r^4 + K_6 r^6) + (T_1 (r^2 + 2 y_u^2) + 2 T_2 x_u y_u) \f$
*/
class Pinhole_Intrinsic_Rational_T2 : public Pinhole_Intrinsic
{
  using class_type = Pinhole_Intrinsic_Rational_T2;

  protected:

    /// center of distortion is applied by the Intrinsics class
    std::vector<double> params_; // K1, K2, K3, K4, K5, K6, T1, T2
  ...
  public:
    ...
    */
    Pinhole_Intrinsic_Rational_T2(
      ...
      : Pinhole_Intrinsic( w, h, focal, ppx, ppy ),
        params_({k1, k2, k3, k4, k5, k6, t1, t2})
    {
    }

    /**
    * @brief Get type of the intrinsic
    * @retval PINHOLE_CAMERA_RATIONAL_T2
    */
    EINTRINSIC getType() const override
    {
      return PINHOLE_CAMERA_RATIONAL_T2;
    }
    ...
    */
    bool updateFromParams( const std::vector<double> & params ) override
    {
      if ( params.size() == 11 )
      {
        *this = Pinhole_Intrinsic_Rational_T2(
                  w_, h_,
                  params[0], params[1], params[2], // focal, ppx, ppy
                  params[3], params[4], params[5], // K1, K2, K3
                  params[6], params[7], params[8], // K4, K5, K6
                  params[9], params[10] );         // T1, T2
        return true;
      }
      else
      {
  ...
  private:
    ...
    * @param params List of parameters to define a rational distortion camera
    * @param p Input point
    ...
    */
    static Vec2 distoFunction( const std::vector<double> & params, const Vec2 & p )
    {
      ...
      const double k4 = params[3], k5 = params[4], k6 = params[5];
      const double t1 = params[6], t2 = params[7];

      const double r2 = p(0)*p(0) + p(1)*p(1);
      const double r4 = r2*r2;
      const double r6 = r4*r2;

      const double rad = (1 + k1*r2 + k2*r4 + k3*r6) / (1 + k4*r2 + k5*r4 + k6*r6);
      const double t_x = t2*(r2 + 2*p(0)*p(0)) + 2*t1*p(0)*p(1);
      const double t_y = t1*(r2 + 2*p(1)*p(1)) + 2*t2*p(0)*p(1);

      return { (p(0)*rad + t_x) - p(0),
               (p(1)*rad + t_y) - p(1) };
    }
};
...
#endif // #ifndef OPENMVG_CAMERAS_CAMERA_PINHOLE_RATIONAL_HPP
