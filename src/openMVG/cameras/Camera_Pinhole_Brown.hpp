...
/**
* @brief Implement a Pinhole camera with 4 radial distortion coefficients and 4 tangential distortion coefficients.
* \f$ x_d = x_u (1 + K_1 r^2 + K_2 r^4 + K_3 r^6 + K_4 r^8) + (T_2 (r^2 + 2 x_u^2) + 2 T_1 x_u y_u) (1 + P_3 r^2 + P_4 r^4) \f$
* \f$ y_d = y_u (1 + K_1 r^2 + K_2 r^4 + K_3 r^6 + K_4 r^8) + (T_1 (r^2 + 2 y_u^2) + 2 T_2 x_u y_u) (1 + P_3 r^2 + P_4 r^4) \f$
*/
class Pinhole_Intrinsic_Brown_K4_T4 : public Pinhole_Intrinsic
{
  using class_type = Pinhole_Intrinsic_Brown_K4_T4;

  protected:

    /// center of distortion is applied by the Intrinsics class
    std::vector<double> params_; // K1, K2, K3, K4, T1, T2, T3, T4
  ...
  public:
    ...
    */
    Pinhole_Intrinsic_Brown_K4_T4(
      ...
      : Pinhole_Intrinsic( w, h, focal, ppx, ppy ),
        params_({k1, k2, k3, k4, t1, t2, t3, t4})
    {
    }

    /**
    * @brief Get type of the intrinsic
    * @retval PINHOLE_CAMERA_BROWN_K4_T4
    */
    EINTRINSIC getType() const override
    {
      return PINHOLE_CAMERA_BROWN_K4_T4;
    }
    ...
    */
    bool updateFromParams( const std::vector<double> & params ) override
    {
      if ( params.size() == 11 )
      {
        *this = Pinhole_Intrinsic_Brown_K4_T4(
                  w_, h_,
                  params[0], params[1], params[2], // focal, ppx, ppy
                  params[3], params[4], params[5], params[6], // K1, K2, K3, K4
                  params[7], params[8], params[9], params[10]); // T1, T2, T3, T4
        return true;
      }
      else
      {
  ...
  private:
    ...
    */
    static Vec2 distoFunction( const std::vector<double> & params, const Vec2 & p )
    {
      ...
      const double t1 = params[4], t2 = params[5], t3 = params[6], t4 = params[7];
      const double r2 = p( 0 ) * p( 0 ) + p( 1 ) * p( 1 );
      const double r4 = r2 * r2;
      const double r6 = r4 * r2;
      const double r8 = r4 * r4;
      const double rad1 = ( k1 * r2 + k2 * r4 + k3 * r6 + k4 * r8 );
      const double rad2 = ( 1 + t3 * r2 + t4 * r4 );
      const double t_x = t2 * ( r2 + 2 * p( 0 ) * p( 0 ) ) + 2 * t1 * p( 0 ) * p( 1 );
      const double t_y = t1 * ( r2 + 2 * p( 1 ) * p( 1 ) ) + 2 * t2 * p( 0 ) * p( 1 );
      return { p( 0 ) * rad1 + t_x * rad2, p( 1 ) * rad1 + t_y * rad2};
    }
};
