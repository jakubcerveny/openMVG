...
/**
* @brief Implement a Pinhole camera with 4 radial distortion coefficients.
* \f$ x_d = x_u (1 + K_1 r^2 + K_2 r^4 + K_3 r^6 + K_4 r^8) \f$
*/
class Pinhole_Intrinsic_Radial_K4 : public Pinhole_Intrinsic
{
  using class_type = Pinhole_Intrinsic_Radial_K4;
  ...
  public:
    ...
    */
    ...
    ~Pinhole_Intrinsic_Radial_K4() override = default;

    /**
    * @brief Tell from which type the embed camera is
    * @retval PINHOLE_CAMERA_RADIAL4
    */
    EINTRINSIC getType() const override
    {
      return PINHOLE_CAMERA_RADIAL4;
    }
    ...
    */
    Vec2 add_disto( const Vec2 & p ) const override
    {
      ...
      const double r_coeff = ( 1. + k1 * r2 + k2 * r4 + k3 * r6 + k4 * r8);
    ...
    */
    bool updateFromParams( const std::vector<double> & params ) override
    {
      if ( params.size() == 7 )
      {
        *this = Pinhole_Intrinsic_Radial_K4(
                  w_, h_,
                  params[0], params[1], params[2], // focal, ppx, ppy
  ...
  private:
    ...
    */
    static inline double distoFunctor( const std::vector<double> & params, double r2 )
    {
      ...
      return r2 * Square( 1. + r2 * ( k1 + r2 * ( k2 + r2 * ( k3 + r2 * k4 ) ) ) );
    }
};
